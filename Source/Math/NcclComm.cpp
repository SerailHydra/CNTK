    ...
    : m_ncclComm(nullptr), m_stream(nullptr)
{
    ...
    */
    ...
    size_t hostId = mpi->NumNodesInUse() / numRanks;
    if (mpi->CurrentNodeRank() % mpi->NumHostsInUse() != 0)
    ...
    res = ncclCommInitRank(&m_ncclComm, numRanks, ncclId, mpi->CurrentNodeRank() % mpi->NumHostsInUse());
    if (res != ncclSuccess)
